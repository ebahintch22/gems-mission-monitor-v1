...
test("GET / affiche le tableau de bord", async () => {
  ...
  assert.match(navigationScriptResponse.text, /siteSearchCoordinateSuggestion/);
  assert.match(navigationScriptResponse.text, /function parseSearchCoordinateCandidate\(/);
  assert.match(navigationScriptResponse.text, /function renderCoordinateSuggestion\(/);
  assert.match(navigationScriptResponse.text, /g2m:site-search-center-map/);
  assert.match(navigationScriptResponse.text, /parseCoordinateNumber\(/);
...
test("la navigation affiche seulement les liens autorises", async () => {
  ...
  assert.match(adminResponse.text, /id="site-search-coordinate-suggestion"/);
  assert.match(adminResponse.text, /data-message-coordinate-title=/);
  ...
  assert.doesNotMatch(agentNavigation, /href="\/cartographie\/extractions-kobo"/);
...
test("POST /admin/settings persiste les parametres et masque les secrets", async () => {
  ...
  const persistedSiteLabelWrapLength = db.prepare("SELECT value FROM settings WHERE key = ?").get("map.site_label_wrap_length");
  ...
  assert.equal(persistedSiteLabelWrapLength.value, "34");
  ...
  assert.match(formResponse.text, /name="settings\[map\.site_label_wrap_length\]"/);
...
test("GET /api/sites/search recherche les sites avec debounce cote client", async () => {
  ...
  assert.match(homeResponse.text, /id="site-search-coordinate-suggestion"/);
...
test("le module sites a visiter importe le CSV et expose l'arborescence", async () => {
  ...
  fs.writeFileSync(csvPath, [
    "ord;CODE;LOCALITE;NOM ETABLISSEMENT;MINISTERE;REGION;PHASE;SURVEY_DATE_PREV;STATUT_OLD;STATUT;SURVEY_DATE_REAL;POINT_GEO;POLYGON_GEO;STATUT_GEOREFERENCEMENT;RAISON_ABANDON_GEOREFERENCEMENT",
    "1;g2m-test-001;COCODY;CHU de Cocody;MSHPCMU;DISTRICT D'ABIDJAN;Phase 1;09/06/2026;OK;done;10/06/2026;;;;",
    "2;g2m-test-002;PLATEAU;College Moderne du Plateau;MENAET;DISTRICT D'ABIDJAN;Phase 1;12/06/2026;NOK;planned;;;;abandoned;CONTOUR_NON_DISCERNABLE",
  ...
  assert.match(scriptResponse.text, /Motif d'abandon/);
  assert.match(scriptResponse.text, /has-georeferencing-abandoned/);
  assert.match(scriptResponse.text, /id = "sites-planning-export-geojson"/);
  assert.match(scriptResponse.text, /function exportSelectedSiteGeoJson\(\)/);
  assert.match(scriptResponse.text, /missing_building_extents/);
  assert.match(scriptResponse.text, /_contour_batiments\.geojson/);
  ...
  assert.match(styleResponse.text, /\.sites-planning-export-dialog\s*\{[\s\S]*position: absolute/);
  assert.match(styleResponse.text, /\.sites-planning-export-panel\s*\{[\s\S]*grid-template-rows: auto auto auto minmax\(0, 1fr\)/);
  assert.match(styleResponse.text, /\.sites-planning-export-table\s*\{[\s\S]*min-height: 0/);
  assert.match(styleResponse.text, /\.sites-planning-osm-workspace\s*\{[\s\S]*grid-template-columns: minmax\(0, 1fr\) minmax\(0, 1fr\)/);
  assert.match(styleResponse.text, /\.sites-planning-osm-panel\s*\{[\s\S]*grid-template-rows: auto auto auto auto minmax\(0, 1fr\)/);
  assert.match(styleResponse.text, /\.sites-planning-osm-report\s*\{[\s\S]*min-height: 0/);
  assert.match(styleResponse.text, /\.sites-planning-osm-list \.tabulator/);
  ...
  assert.match(styleResponse.text, /\.sites-planning-table tr\.has-georeferencing-abandoned/);
  assert.match(styleResponse.text, /\.sites-planning-table tr\.is-selected-planning-site\s*\{[\s\S]*background: #0f766e/);
  assert.match(styleResponse.text, /\.sites-planning-table \.tabulator-row\.has-georeferencing-abandoned\s*\{[\s\S]*background: #343a40/);
  ...
  assert.equal(sitesResponse.body.sites.find((site) => site.code === "g2m-test-002").georeferencing_status, "abandoned");
  assert.equal(sitesResponse.body.sites.find((site) => site.code === "g2m-test-002").georeferencing_abandon_reason, "CONTOUR_NON_DISCERNABLE");
  assert.equal(sitesResponse.body.georeferencing_abandon_reasons.CONTOUR_NON_DISCERNABLE, "Contour non discernable");
...
test("GET /cartographie expose l'espace SIG et ses points cartographiques", async () => {
  ...
  assert.match(response.text, /id="sig-administrative-choices-data"/);
  ...
  assert.match(response.text, /id="sig-table-title"/);
  assert.match(response.text, /sites affich/);
  ...
  assert.match(scriptResponse.text, /function siteLabelWrapLength\(\)/);
  assert.match(scriptResponse.text, /geometryImportConfig\.siteLabelWrapLength/);
  ...
  assert.match(scriptResponse.text, /id="sig-sites-planning-export-dialog" hidden/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-export-table"/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-export-run" disabled/);
  assert.match(scriptResponse.text, /class="sites-planning-osm-workspace"/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-osm-selection-count"/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-osm-result-count"/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-osm-site-details"/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-plan-open" hidden disabled/);
  assert.match(scriptResponse.text, /id="sig-sites-planning-export-geojson" hidden disabled/);
  ...
  assert.match(scriptResponse.text, /async function exportSelectedPlanningSiteGeoJson\(\)/);
  assert.match(scriptResponse.text, /function openSitesPlanningExportDialog\(\)/);
  assert.match(scriptResponse.text, /async function exportSelectedPlanningSitesGeoJson\(\)/);
  assert.match(scriptResponse.text, /function buildPlanningSitesContoursExportGeoJson/);
  assert.match(scriptResponse.text, /function buildPlanningSitesBuildingsExportGeoJson/);
  assert.match(scriptResponse.text, /_contours_sites\.geojson/);
  assert.match(scriptResponse.text, /_emprises_batiments\.geojson/);
  assert.match(scriptResponse.text, /function buildSelectedPlanningSiteExportGeoJson/);
  assert.match(scriptResponse.text, /downloadPlanningSiteGeoJson\(geojson, selectedPlanningSiteExportFilename/);
  assert.match(scriptResponse.text, /Export impossible : aucune emprise de batiment importee pour ce site\./);
  ...
  assert.match(scriptResponse.text, /let sitesPlanningOsmSitesTable = null/);
  assert.match(scriptResponse.text, /let sitesPlanningOsmResultsTable = null/);
  assert.match(scriptResponse.text, /new Tabulator\(list/);
  assert.match(scriptResponse.text, /new Tabulator\(report/);
  assert.match(scriptResponse.text, /formatter: "rowSelection"/);
  assert.match(scriptResponse.text, /rowDblClick\(event, row\)/);
  assert.match(scriptResponse.text, /function selectedSitesPlanningOsmTableData\(\)/);
  assert.match(scriptResponse.text, /getSelectedData/);
  assert.match(scriptResponse.text, /tabulator-selected/);
  assert.match(scriptResponse.text, /headerFilter: "input"/);
  assert.match(scriptResponse.text, /name="sig_planning_osm_scope" value="missing" checked/);
  assert.match(scriptResponse.text, /name="sig_planning_osm_scope" value="all"/);
  assert.match(scriptResponse.text, /function sitesPlanningHasOsmBuildings\(site\)/);
  assert.match(scriptResponse.text, /function sitesPlanningOsmIdsToProcess\(selectedIds\)/);
  assert.match(scriptResponse.text, /function confirmLargeSitesPlanningOsmImport\(count\)/);
  assert.match(scriptResponse.text, /if \(count < 10\)/);
  assert.match(scriptResponse.text, /const processIds = sitesPlanningOsmIdsToProcess\(selectedIds\)/);
  ...
  assert.match(scriptResponse.text, /function normalizeVisitedSiteSortValue\(point, field\)/);
  assert.match(scriptResponse.text, /function formatVisitedSiteSortValue\(point, field\)/);
  assert.match(scriptResponse.text, /localeCompare\(String\(bValue\), locale/);
  ...
  assert.match(scriptResponse.text, /document\.getElementById\("sig-summary-toggle"\)\?\.addEventListener\("click"/);
  assert.match(scriptResponse.text, /function updateSelectedSiteTableRowStyles\(\)/);
  assert.match(scriptResponse.text, /function updateVisitedSitesTableTitle\(count\)/);
  assert.match(scriptResponse.text, /updateVisitedSitesTableTitle\(visiblePoints\.length\)/);
  assert.match(scriptResponse.text, /replace\("\{\{count\}\}", count\)/);
  ...
  assert.match(scriptResponse.text, /const restored = hasSubmissionQuery \? false : await restoreCartographyContext\(savedContext\)/);
  ...
  assert.match(scriptResponse.text, /function siteLabelText\(point\)/);
  assert.match(scriptResponse.text, /function siteLabelHtml\(label\)/);
  assert.match(scriptResponse.text, /fixedWrapSiteLabel\(label, siteLabelWrapLength\(\)\)/);
  assert.match(scriptResponse.text, /function fixedWrapSiteLabel\(label, maxLineLength = 30\)/);
  assert.match(scriptResponse.text, /if \(currentLine\.length >= maxLineLength\)/);
  assert.match(scriptResponse.text, /currentLine = `\$\{currentLine\} \$\{word\}`/);
  ...
  assert.match(scriptResponse.text, /marker\.setTooltipContent\(siteLabelHtml\(label\)\)/);
  assert.match(scriptResponse.text, /className: "sig-site-label"/);
  assert.match(scriptResponse.text, /map\.on\("moveend zoomend", updateSiteMarkerLabels\)/);
  ...
  assert.match(scriptResponse.text, /table\.redraw\(true\)/);
  assert.match(scriptResponse.text, /refreshPalLayout\(\)/);
  ...
  assert.match(scriptResponse.text, /let contextPersistenceFrozenUntil = 0/);
  assert.match(scriptResponse.text, /Date\.now\(\) < contextPersistenceFrozenUntil/);
  assert.match(scriptResponse.text, /function saveCartographyContextNow\(\)/);
  assert.match(scriptResponse.text, /contextPersistenceFrozenUntil = Date\.now\(\) \+ 1000/);
  assert.match(scriptResponse.text, /window\.setTimeout\(function \(\) \{[\s\S]*contextPersistenceFrozenUntil = 0;[\s\S]*saveCartographyContextNow\(\);[\s\S]*\}, 1000\)/);
  ...
  assert.match(styleResponse.text, /\.sig-map-footer-band\s*\{[\s\S]*grid-template-columns: minmax\(180px, 300px\) minmax\(220px, 1fr\) minmax\(220px, 310px\)/);
  assert.match(styleResponse.text, /\.sig-map-footer-band\s*\{[\s\S]*background: transparent/);
  assert.match(styleResponse.text, /\.sig-map-footer-band\s*\{[\s\S]*border: 0/);
  assert.match(styleResponse.text, /\.sig-map-footer-band\s*\{[\s\S]*opacity: 1/);
...
test("GET /cartographie/extractions-kobo affiche la revue dediee des geometries Kobo", async () => {
  const testAssetDir = path.join(__dirname, "..", "data", "kobo-assets", "asset-test", "submission-test");
  ...
  const assetResponse = await request(app)
    .get("/cartographie/extractions-kobo/assets/asset-test/submission-test/photo-test.jpg")
    .set("Cookie", gisCookie);
  const deniedAssetResponse = await request(app)
    .get("/cartographie/extractions-kobo/assets/asset-test/submission-test/photo-test.jpg")
    .set("Cookie", agentCookie);
  const invalidAssetResponse = await request(app)
    .get("/cartographie/extractions-kobo/assets/asset-test/submission-test/photo-test.txt")
    .set("Cookie", gisCookie);
  const scriptResponse = await request(app).get("/js/kobo-geometries-review.js");
  fs.rmSync(path.join(__dirname, "..", "data", "kobo-assets", "asset-test"), { recursive: true, force: true });
  ...
  assert.equal(deniedAssetResponse.status, 403);
  ...
  assert.match(response.text, /id="kobo-submission-tree-search"/);
  ...
  assert.match(scriptResponse.text, /filterTreeNodes/);
  assert.match(scriptResponse.text, /normalizeSearchText/);
  assert.match(scriptResponse.text, /localPhotosForSubmission/);
  assert.match(scriptResponse.text, /kobo-submission-photo-grid/);
...
test("GET /media/:id/view genere une URL signee Wasabi pour un utilisateur autorise", async () => {
  const previousEnv = {
  ...
  process.env.WASABI_ACCESS_KEY_ID = "access-test";
  process.env.WASABI_SECRET_ACCESS_KEY = "secret-test";
  process.env.WASABI_REGION = "eu-central-1";
  process.env.WASABI_BUCKET = "g2m-media-test";
  ...
  try {
    const media = MediaFile.create({
      ...
      object_key: "media/test/2026/07/media-route-test/original/photo.jpg",
    ...
    const gisCookie = await loginTestUser({
      email: "gis.media-view@g2m.test",
      role: "specialiste_gis"
    });
    const agentCookie = await loginTestUser({
      email: "agent.media-view-denied@g2m.test",
      role: "agent"
    });

    const anonymousResponse = await request(app).get(`/media/${media.id}/view`);
    const deniedResponse = await request(app)
    ...
    assert.equal(anonymousResponse.status, 302);
    assert.equal(anonymousResponse.headers.location, `/login?next=%2Fmedia%2F${media.id}%2Fview`);
    assert.equal(deniedResponse.status, 403);
    assert.equal(response.status, 302);
    assert.match(response.headers.location, /^https:\/\/s3\.eu-central-1\.wasabisys\.com\/g2m-media-test\/media\/test\/2026\/07\/media-route-test\/original\/photo\.jpg\?/);
    assert.match(response.headers.location, /X-Amz-Algorithm=AWS4-HMAC-SHA256/);
    assert.match(response.headers.location, /response-content-disposition=inline/);
  } finally {
...
test("GET /cartographie?lang=en utilise les ressources anglaises SIG", async () => {
  ...
  assert.match(response.text, /Visited sites/);
