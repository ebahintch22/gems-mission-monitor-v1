...
const { extractKoboGeometryBatchV2 } = require("../services/koboGeometryExtractor");
...
const DEFAULT_OUTPUT = "KBase-docs/kobo-data-sample/kobo-response-2026-07-02T09-48-01-0072-geometries-normalized-v2.json";
...
function main() {
  ...
  const output = extractKoboGeometryBatchV2(sourcePayload, strategy);
...
function resolveBatchExecutionPaths(args) {
  ...
  const outputPath = args.output
    ...
    : path.join(batchPath, "02_output", `${path.basename(sourcePath, path.extname(sourcePath))}-geometries-normalized-v2.json`);
...
function printHelpAndExit() {
  console.log([
    "Usage:",
    "  node scripts/extract-kobo-geometries-v2.mjs [source.json] [strategy.json] [output.json]",
    "  node scripts/extract-kobo-geometries-v2.mjs --batch <batch> [--source <json>] [--strategy <json>] [--output <json>]",
    "",
    "Mode --batch:",
    ...
    "  - sortie automatique : 02_output/<source>-geometries-normalized-v2.json",
    "  - V2 : ajoute les attributs batimentaires Kobo bruts dans les proprietes des batiments"
  ].join("\n"));
  process.exit(0);
}
